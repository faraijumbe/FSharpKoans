    ...
    *)
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec add1 somelist = 
                match somelist with 
                |[] -> []
                |first::rest -> (first + 1):: add1 rest
            add1 xs
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec double somelist = // write a function which doubles each element
                match somelist with 
                |[] -> []
                |first::rest -> (first * 2)::double rest
            double xs
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec genericMap somelist = 
                match somelist with 
                |[] -> []
                |first::rest-> f (first) :: genericMap rest
            genericMap xs
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filt somelist =
                match somelist with
                |[] -> []
                |first::rest -> match f(first) with
                                |true -> first::filt rest
                                |false -> filt rest
            filt xs
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
                let rec makeOdd somelist =
                    match somelist with
                    |[] -> []
                    |first::rest -> match first % 2 = 1 with
                                    |true -> first :: makeOdd rest
                                    |false -> makeOdd rest
                makeOdd xs
   ...
   // ... you can make a function to do that, right? ^_^.
    ...
    let ``12 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec genericFilter somelist = // write a function which filters based on the specified criteria
                match somelist with 
                |[] -> []
                |first::rest -> match f(first) with
                                |true -> first::genericFilter rest
                                |false -> genericFilter rest
            genericFilter xs
        filter (fun x -> x > 19) [9; 5; 23; 66; 4] |> should equal [23; 66]
        filter (fun x -> String.length x = 4) ["moo"; "woof"; "yip"; "nyan"; "meow"]
        |> should equal ["woof"; "nyan"; "meow"]
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let rec fold initialState xs =
            match xs with
            |[] -> initialState
            |first::rest -> fold (initialState + first) rest
    ...
    let ``16 Folding, the hard way`` () =
        let rec fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            match xs with 
            |[] -> initialState
            |first::rest -> fold f (f initialState first) rest
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) = 
            let rec reverse xs acc =
                ...
                |[] -> acc
                |chr::rest -> reverse rest (chr::acc)
            let rec genericPartition list1 list2 f someList = 
                match someList with
                |[] -> reverse list1 [], reverse list2 []
                |first::rest -> match f first with 
                                |true -> genericPartition (first::list1) list2 f rest
                                |false ->genericPartition list1 (first::list2) f rest
            genericPartition [] [] f xs
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
           let rec genericInit length f index result =
               match length = 0 with 
               |true -> result
               |false -> match f (index) with
                         |value -> value::genericInit (length-1) f (index+1) result
           genericInit n f 0 []
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let rec tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            match xs with // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
            |[] -> None
            |first::rest -> match p first with
                            |true -> Some(first)
    ...
    let ``22 tryPick: find the first matching element, if any, and transform it`` () =
        let rec tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            match xs with // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
            |[] ->None
            |first::rest -> match p first with 
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let rec choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            match xs with // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
            |[] -> []
            |first::rest -> match p first with 
                            |Some(value) -> value::choose p rest
                            |None -> choose p rest
        let f x =
            match x<=45 with
            | true -> Some(x*2)
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        ...
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec genericmapi f index xs = 
                match xs with // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
                |[] -> []
                |first::rest -> match f index first with
                                |value -> value::genericmapi f (index + 1) rest 
            genericmapi f 0 xs
