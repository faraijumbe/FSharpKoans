...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BA _ -> "A thinker, eh?"
        randomOpinion (BSc(InformationSystem, ComputerScience)) |> should equal "Good choice!"
        randomOpinion (BSc(Linguistics, Philosophy)) |> should equal "!!SCIENCE!!"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node("node 1", Node("Node 2", Node("Node 3", Node("Node 4", Empty,Empty), Empty), Empty), Empty)// <-- you may want to spread this over multiple lines and/or let-bindings ...!
